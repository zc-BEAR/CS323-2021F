...
public class Parser {
    ...
    public static void procedure(Character nonterminal) throws Exception {
        ...
        int x = nonterminals.indexOf(nonterminal);
        int y = terminals.indexOf(inputBuffer.get(0));
        if (x == -1 || y == -1){
            System.out.println("error");
            System.exit(0);
        }
        Production production = parsingTable[x][y];
        if (production == null){
            System.out.println("error");
            System.exit(0);
        }
        productionsToApply.add(production);
        for (Character c : production.body) {
            if (nonterminals.contains(c)) {
                procedure(c);
            } else if (c.equals(inputBuffer.get(0))) {
